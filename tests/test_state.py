...
def test_mark_synced_excludes_from_future(state_db_path):
    state = StateStore(state_db_path)
    state.mark_synced("op-1", product_id="p1", ombor_event_id="e1", batch_count=3)
...
def test_upsert_updates_status(state_db_path):
    ...
    state.mark_synced("op-5", product_id="p1", ombor_event_id="e1", batch_count=2)
...
def test_state_persists_across_reopen(state_db_path):
    ...
    state1.mark_synced("op-1", product_id="p1", ombor_event_id="e1", batch_count=1)
    state1.close()
